"""
test_blake3.py - Blake3 tests adapted for browser/Pyodide environment.

Original tests from blake3-py, adapted for hio test runner.
Some tests removed that require subprocess or mmap (not available in browser).
"""
...
try:
    ...
    # Fallback for different path resolution in Pyodide
    VECTORS = json.load(open("test_vectors.json"))
...
# test_example_dot_py removed - requires subprocess, not available in browser
...
# test_mmap removed - requires filesystem mmap, not available in browser


# List of all test functions for the test loader
ALL_TESTS = [
    test_vectors,
    test_buffer_types,
    test_key_types,
    test_invalid_key_lengths,
    test_int_array_fails,
    test_strided_array_fails,
    test_string_fails,
    test_constants,
    test_xof,
    ...
    test_usedforsecurity_ignored,
    test_context_must_be_str,
    test_buffers_released,
    test_reset,
    test_output_overflows_isize,
    test_module_name,
]
